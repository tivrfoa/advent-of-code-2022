...
fn append(nodes: &mut Vec<usize>, at: usize, n: usize) {
	nodes[n] = nodes[at];
	nodes[at] = n;
}

fn popn(nodes: &mut Vec<usize>, at: usize, n: usize) -> usize {
	let mut next: usize = nodes[at];
	let taken = next;
...
fn pushn(nodes: &mut Vec<usize>, at: usize, node: usize, n: usize) {
	let head = node;
...
pub fn part2(input: String) -> String {
	let mut cups: Vec<usize> = vec![0; 1_000_001];
	...
	for i in 10..=N {
		append(&mut cups, prev, i);
		prev = i;
	}

	for _ in 0..N * 10 {
		let taken = popn(&mut cups, current, 3); // cups[current].popn(&cups, 3);
		let taken_n = {
			let taken2 = cups[taken];
			let taken3 = cups[taken2];
			[taken, taken2, taken3]
		};
		...
		pushn(&mut cups, current_label, taken, 3); // cups[current_label].pushn(&cups, taken, 3);
