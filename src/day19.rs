...
fn dfs(bp: &Blueprint, cache: &mut HashMap<State, u16>, state: State) -> u16 {
    ...
    'lr:
    for (btype, recipe) in bp.recipes.iter().enumerate() {
        ...
        for (ramt, rtype) in recipe {
            ...
            if *ramt > state.resources[*rtype] {
                //let cd1 = ceil_div(*ramt - state.resources[*rtype], state.robots[*rtype]);
                //let cd2 = ceil_div2(*ramt - state.resources[*rtype], state.robots[*rtype]);
                //wait = wait.max(cd2);
                wait = wait.max((*ramt - state.resources[*rtype])
                    .div_ceil(state.robots[*rtype]));
            }
        }
